    ...
    <div className="fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5">
      ...
      <div className="flex items-center gap-8">
        ...
        {!user ? (
          <button
            onClick={openSignIn}
            className="px-4 py-1 sm:px-7 sm:py-2 bg-rimary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer"
          >
            Login
          </button>
        ) : (
      ...
      <MenuIcon
        onClick={() => setIsOpen(!isOpen)}
        className="max-md:ml-4n md:hidden w-8 h-8 cursor-pointer"
      />
    </div>
  );
}
